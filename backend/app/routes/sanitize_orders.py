...
from app.sanitize_rules import ConflictError, apply_transition, ensure_no_active_order
...
def create_sanitize_order():
    ...
    try:
        ...
        try:
            ensure_no_active_order(db, room.id)
...
def transition_sanitize_order(order_id: int):
    ...
    try:
        try:
            ...
            return validation_error_response(err)
        order = db.query(SanitizeOrder).filter(SanitizeOrder.id == order_id).first()
        if not order:
            return jsonify({"detail": "消杀单不存在"}), 404
        room = db.query(Room).filter(Room.id == order.room_id).first()
        if not room:
            return jsonify({"detail": "出菇室不存在"}), 400
        try:
            apply_transition(db, order, data["to"], room)
        except ConflictError as err:
            db.rollback()
            return jsonify({"detail": str(err)}), 409
        db.commit()
        db.refresh(order)
        return jsonify(out_schema.dump(order))
    finally:
        db.close()

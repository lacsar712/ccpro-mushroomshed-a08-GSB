...
const methods: { value: SanitizeMethod; label: string }[] = [
  { value: 'uv', label: '紫外线 (uv)' },
  { value: 'chemical', label: '化学药剂 (chemical)' },
]
...
export default function SanitizeOrders() {
  ...
  async function transition(id: number, toStatus: SanitizeStatus) {
    if (toStatus === 'void' && !confirm('确认作废该消杀工单？')) return
    setError('')
    try {
  ...
  return (
    <div>
      <header class="page-header">
        <h1>消杀工单</h1>
        <p class="muted">open → doing → done；非终态可 void。开工自动置房态为 sanitize，完工需窗口内环境记录</p>
      </header>
      ...
      <form class="panel form-grid" onSubmit={onSubmit}>
        ...
        <label>
          ...
          <select
            value={form().method}
            onChange={(e) => setForm({ ...form(), method: e.currentTarget.value as SanitizeMethod })}
          >
                ...
                <tr>
                  ...
                  <td>
                    <div class="row-actions">
                      {o.status === 'open' && (
                        <button type="button" class="btn ghost" onClick={() => transition(o.id, 'doing')}>
                          开工
                        </button>
                      )}
                      {o.status === 'doing' && (
                        <button type="button" class="btn ghost" onClick={() => transition(o.id, 'done')}>
                          完工
                        </button>
                      )}
                      {(o.status === 'open' || o.status === 'doing') && (
                        <button type="button" class="btn ghost" onClick={() => transition(o.id, 'void')}>
                          作废
                        </button>
                      )}
                      {(o.status === 'done' || o.status === 'void') && '—'}
                    </div>
                  </td>
                </tr>
              )}
